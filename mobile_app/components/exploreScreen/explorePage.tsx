...
function getMediaPreviewUrl(media?: MediaLike | null) {
   ...
   *
   * Image:
   * Prefer optimized feed URL,
   * then original URL.
   */
  if (isVideoMedia(media)) {
    return media.thumbnail_url ?? media.feed_url ?? media.url ?? undefined;
  }

  return media.feed_url ?? media.url ?? media.thumbnail_url ?? undefined;
}
...
export default function ExploreScreen() {
  ...
  // ==================================================
  // FLATTEN PAGINATED LISTINGS
  // ==================================================

  const listings = listingsData?.pages.flatMap((page) => page.results) ?? [];
  ...
  const displayedListings = useMemo(() => {
    ...
    if (query) {
      result = result.filter((item) => {
        ...
        const category = item.category?.name?.toLowerCase() ?? "";
        const seller = item.seller?.display_name?.toLowerCase() ?? "";
        const username = item.seller?.username?.toLowerCase() ?? "";
        const city = item.location?.city?.toLowerCase() ?? "";
        const area = item.location?.area?.toLowerCase() ?? "";
  ...
  return (
    <SafeAreaView style={styles.container} edges={["top"]}>
      ...
      {isFetchingListings && !isFetchingNextPage && (
